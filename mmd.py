...
def mmd(X, prototypes, gamma):
    ...
    mmd2 = 1/(prototypes.shape[0]**2) * spp - 2/(prototypes.shape[0]
                                                 * X.shape[0]) * spdp + 1/(X.shape[0]**2) * sdp
...
def mmd_joblib(X, prototypes, gamma):
    ...
    # Needs to be multiplied with 2 (lower part of proximity matrix)
    # Sum of identity matrix needs to be added (1 on diag)
    ans = joblib.Parallel(n_jobs=12)(joblib.delayed(
        k)(p[0], p[1], gamma) for p in prototypes[cmb])
    utm = np.sum(ans)
    spp = utm * 2 + prototypes.shape[0]
    ...
    # Needs to be multiplied with 2 (lower part of proximity matrix)
    # Sum of identity matrix needs to be added (1 on diag)
    ans = joblib.Parallel(n_jobs=12)(
        joblib.delayed(k)(x[0], x[1], gamma) for x in X[cmb])
    utm = np.sum(ans)
    sdp = utm * 2 + X.shape[0]
